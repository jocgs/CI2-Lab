...
export function AvatarShopCard({ avatar, unlocked, active, coins }: Props) {
  ...
  return (
    <div className={`flex flex-col items-center gap-3 rounded-2xl border p-5 transition-shadow ${active ? "border-[var(--brand)] bg-[var(--brand-soft)] shadow-md" : "border-[var(--border)] bg-[var(--surface)]"}`}>
      <div className="relative h-28 w-28 overflow-hidden rounded-2xl bg-[var(--background)]">
      ...
      {unlocked ? (
        <button
          ...
          disabled={isPending}
          className={`w-full rounded-xl px-4 py-2 text-sm font-medium transition-colors disabled:opacity-60 ${active ? "bg-[var(--surface)] border border-[var(--border)] text-[var(--muted)] hover:border-red-300 hover:text-red-500" : "bg-[var(--brand)] text-white hover:opacity-90"}`}
        >
        ...
        <button
          ...
          className="w-full rounded-xl bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 disabled:opacity-50 transition-colors"
        >
